...
//
// OpenFGAAuthorizer adapts the official OpenFGA Go SDK to this project's
// Checker interface.  Domain code depends on the interface, not on SDK types.
//
...
// Authorizer adapts the official OpenFGA Go SDK to this repo's Checker interface.
// It can also write tuples, making it a full replacement for the in-process
// graph evaluator + in-memory store when running against a real OpenFGA server.
type Authorizer struct {
	client sdkClient
}

// New creates an Authorizer backed by the official SDK.
...
// Check satisfies the Checker interface by asking OpenFGA to evaluate the
// relationship graph remotely.
func (a *Authorizer) Check(ctx context.Context, req shared.CheckRequest) (shared.CheckResult, error) {

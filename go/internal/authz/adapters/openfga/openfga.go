...
func (s *Service) DeleteTuples(ctx context.Context, tuples []shared.TupleKey) error {
	if len(tuples) == 0 {
		return nil
	}
...
func (s *Service) ListTuples(ctx context.Context, filter ...authz.TupleFilter) ([]shared.TupleKey, error) {
	...
	out := make([]shared.TupleKey, 0, len(tuples))
	for _, t := range tuples {
		key := t.GetKey()
		out = append(out, shared.TupleKey{
			Object:   shared.Object(key.GetObject()),
			Relation: shared.Relation(key.GetRelation()),
			User:     shared.Subject(key.GetUser()),
		})
	}
	return out, nil
}

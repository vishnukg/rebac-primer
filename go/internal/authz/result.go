...
func (r Result[T]) Unwrap() T {
	if !r.ok {
		panic(fmt.Sprintf("authz.Result.Unwrap on a failure: %v", r.err))
	}
	return r.value
}
...
//
// The second type parameter U is inferred by the compiler from f's return type.
// This is Go's equivalent of .map() on TypeScript's Result or Option types.
//

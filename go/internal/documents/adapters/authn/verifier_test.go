...
func newVerifier() *authn.DemoTokenVerifier {
	return authn.New(map[string]authn.TokenClaims{
...
func TestVerifier_GivenValidBearerToken_WhenVerified_ThenReturnsAuthenticatedUser(t *testing.T) {
	...
	if user.Subject != shared.User("alice") {
		t.Errorf("Subject = %q, want %q", user.Subject, shared.User("alice"))
	}

...
export class TerminalClient {
  ...
  async run(): Promise<void> {
    ...
    this.config.write("Actors: alice can edit, bob can read, chandra is denied by default.");
  ...
  private async readRoadmap(): Promise<void> {
    const actorId = await this.config.terminal.question("Actor id: ");
    const document = await this.config.client.readDocument("roadmap", actorId);
    this.config.write(`\n${document.title}`);
  ...
  private async updateRoadmap(): Promise<void> {
    ...
    const document = await this.config.client.updateDocument("roadmap", actorId, body);

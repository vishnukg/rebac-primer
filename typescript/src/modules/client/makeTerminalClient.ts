import type { DocumentsClient, QuestionTerminal, TerminalClient } from "./types.ts";
...
const makeTerminalClient = ({ client, terminal, write }: TerminalClientCfg): TerminalClient => {
  const run = async (): Promise<void> => {
    ...
    while (running) {
      ...
      if (choice === "1") {
        await readRoadmapDocument();
      } else if (choice === "2") {
        await updateRoadmapDocument();
      } else if (choice === "3") {
        running = false;
      } else {
  ...
  const readRoadmapDocument = async (): Promise<void> => {
    try {
      ...
      const document = await client.readDocument("roadmapDocument", actorId);
      write(`\n${document.title}`);
      write(document.body);
      write(`updated by ${document.updatedBy}`);
    } catch (error) {
  ...
  const updateRoadmapDocument = async (): Promise<void> => {
    try {
      ...
      const document = await client.updateDocument("roadmapDocument", actorId, body);
      write(`Updated ${document.id}; updated by ${document.updatedBy}`);
    } catch (error) {
      write(`Denied: ${error instanceof Error ? error.message : "unknown error"}`);
    }
  };

...
import {
    ...
    composeInProcessAuthzClient,
} from "./fixtures.ts";

const q = new URLSearchParams();

// Builds a fresh handler (and fresh in-memory stores) for each test.
const composeHandler = (): DocumentsHttpHandler => {
...
const composeHandlerWithDocument = async (): Promise<DocumentsHttpHandler> => {
...
describe("Documents service — GET /documents/:id", () => {
    it("allows a workspace viewer to read (permission inherited via workspace)", async () => {
        ...
        const handler = await composeHandlerWithDocument();

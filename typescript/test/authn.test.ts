...
describe("makeDemoTokenVerifier", () => {
  ...
  it("rejects missing and unknown tokens", async () => {
    ...
    await expect(authenticator.verifyAccessToken(undefined)).rejects.toThrow("Missing Authorization header");
    await expect(authenticator.verifyAccessToken("Bearer nope")).rejects.toThrow("Invalid access token");
  });
});

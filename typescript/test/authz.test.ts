...
import {
  ...
  tuple,
  user,
} from "../src/modules/authz/index.ts";
import {
  alice,
  bob,
  casey,
...
} from "../src/modules/fixtures/index.ts";
...
describe("makeGraphAuthorizer", () => {
  it("allows alice to edit through team membership and workspace inheritance", async () => {
    ...
    const result = await authorizer.check({ user: alice, relation: "can_edit", object: roadmapDocument });
  ...
  it("lets bob read as a workspace viewer but denies editing", async () => {
    ...
    await expect(authorizer.check({ user: bob, relation: "can_read", object: roadmapDocument }))
      .resolves.toMatchObject({ allowed: true });
    await expect(authorizer.check({ user: bob, relation: "can_edit", object: roadmapDocument }))
  ...
  it("treats team admins as team members", async () => {
    ...
    const result = await authorizer.check({ user: casey, relation: "member", object: platformTeam });

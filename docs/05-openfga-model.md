...
- Go: `go/internal/authzservice/adapters/graph/permissionmodel.go`
...
2. Add the expansion rule in `expandDocument` in `go/internal/authzservice/adapters/graph/evaluator.go`
3. Add a test in `go/internal/authzservice/adapters/graph/evaluator_test.go` following the AAA pattern
